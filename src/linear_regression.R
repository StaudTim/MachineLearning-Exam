...
# Model
model <- lm( median_house_value ~ longitude + latitude + housing_median_age + 
               beds_per_room + population + households + 
               median_income + ocean_proximity, data=data.train)

# Calculate forecast quality
X.test <- data.test[,c("longitude","latitude","housing_median_age",
                       "beds_per_room","population",
                       "households", "median_income", "ocean_proximity")]
prog <- predict(model,X.test)
y.test <- data.test[,"median_house_value"]
mad(y.test-prog, constant = 1) # MAE = 30830.8
